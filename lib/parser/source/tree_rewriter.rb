module Parser
  module Source
    ...
    # ## Overlapping ranges:
    #
    # Any two actions on overlapping ranges will fail and raise a `ClobberingError`,
    ...
    # * delete ', :world'
    #
    # The overlapping ranges are merged and `':hello, :world'` will be deleted.
    ...
    # Exception: rewrites done on exactly the same range (covered next).
    #
    # Example:
    # * replace ', ' by ' => '
    ...
    #
    # A containing replacement will swallow the contained rewrites
    ...
    class TreeRewriter
     ...
     #
      ...
      def trigger_policy(event, highlights: [], range: highlights.inject(:join), **arguments)
        action = @policy[event] || :raise
        diag = Parser::Diagnostic.new(POLICY_TO_LEVEL[action], event, arguments, range, highlights)
