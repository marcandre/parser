    ...
    class TreeRewriter::Action
      ...
      def relationship_with(action)
        if action.range == @range
          ...
          :sibbling
        elsif @range.contains?(action.range)
          :parent
        elsif @range.contained?(action.range)
          :child
        elsif !action.insertion? && !insertion?
          @enforcer.call(:crossing_deletions) { {highlights: [action.range, @range]} }
          :fusible
        else
          @enforcer.call(:crossing_insertions) { {highlights: [action.range, @range]} }
        end
      end
      ...
      def swallow(children)
        @enforcer.call(:swallowed_insertions) do
          ...
          {range: @range.begin, highlights: insertions.map(&:range) << range.end} unless insertions.empty?
        end
        []
      end
    end
